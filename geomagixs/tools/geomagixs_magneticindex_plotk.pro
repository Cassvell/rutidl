...
;##############################################################################
;##############################################################################
PRO geomagixs_magneticindex_plotk, initial, final, STATION=station, $
                                       QUIET=quiet, $
                                       FORCE_ALL=force_all, $
                                       REAL_TIME=real_time, $
                                       N_DAYS=n_days
...
;##############################################################################
        ...
        FOR i = 0ll, N_ELEMENTS(make_update_file)-1 DO BEGIN
                IF make_update_file[i] EQ 1 THEN BEGIN
                        ...
                        ENDIF ELSE making_kplot, [tmp_year, tmp_month, tmp_day], N_DAYS=n_days, STATION=station, QUIET=quiet, REAL_TIME=real_time
                ENDIF
        ENDFOR
