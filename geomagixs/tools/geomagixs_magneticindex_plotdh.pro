...
;##############################################################################
        ...
        ;        print, 
        ;ENDIF
        READS, k_index_data, tmp_var, FORMAT='(25(X, F8))';'(8(I3,X), I3)'
        result.delta_H[*] = tmp_var[0].z[0:23]
        result.sigma_H[*] = tmp_var[1].z[0:23]
        RETURN, result
END
...
;##############################################################################
;##############################################################################
PRO making_deltaH_plot, initial, final, STATION = station, $
                                    REAL_TIME = real_time, $
                                    QUIET = quiet, $
                                    PNG = png, $
                                    JPEG = jpeg, $
...
;##############################################################################
        ...
        IF capable_to_plot NE N_ELEMENTS(data_file_name) AND NOT keyword_set(quiet) THEN BEGIN 
                ...
                PRINT, FORMAT="('                missing GMS_YYYYMMDD.delta_H.',A,' impossible to plot all data.')", extention
                Error.value[1] += 1
        ...
        FOR i = 0, N_ELEMENTS(exist_data_file)-1 DO BEGIN
                IF exist_data_file[i] EQ 1 THEN BEGIN
                        ...
                        dH_data[i*24:(i+1)*24-1]       = tmp_data.delta_H[*]
                ...
                ENDIF ELSE BEGIN
                        dH_sigma_data[i*24:(i+1)*24-1] = 999999.
                        dH_data[i*24:(i+1)*24-1]       = 999999.
                ENDELSE
        ENDFOR
...
;##############################################################################
        ...
        IF gms[system.gms].name EQ 'planetary' THEN BEGIN
                ...
                K_axis_title  = 'Dst [nT]';'Kmex!N index'
                Plot_title    = ' Dst index'; +' (GMS: '+kmex_GMS_name[kmex_GMS]+')'
                Plot_title    = keyword_set(real_time) ? 'Real-Time'+Plot_title : 'Definitive'+Plot_title
        ENDIF ELSE BEGIN
                ...
                Time_title    = 'UTC [Begin: '+string(initial_year, initial_month, initial_day, FORMAT='(I4,"/",I02,"/",I02)')+' 00:00 UTC]'
                K_axis_title  = TEXTOIDL('\Delta')+'H!D'+STRUPCASE(gms[system.gms].code)+'!N [nT]';'Kmex!N index'
                Plot_title    = ' Hourly-averaged variations of B!DH!N'; +' (GMS: '+kmex_GMS_name[kmex_GMS]+')'
...
;!P.FONT = 0
        ...
        plot, time, dH_data, psym=6, /NoDATA, BACKGROUND = blanco, COLOR=negro, MIN_VALUE=y_minimum, MAX_VALUE=y_maximum, $
        ...
        oplot, time, dh_data, thick=4, color=negro, MIN_VALUE=y_minimum, MAX_VALUE=y_maximum;, linestyle=0
        ...
        file_name = gms[system.gms].code+'_'+string(initial_year,initial_month,initial_day,file_number, FORMAT='(I4,I02,I02,"_",I02,"days.dH")')+extention
...
;##############################################################################
;##############################################################################
PRO making_dhplot, initial, final, N_DAYS=n_days, STATION=station, QUIET=quiet, REAL_TIME=real_time, $
                                PNG = png, $
                                JPEG = jpge, $
                                DIR = dir
...
;##############################################################################
        ...
        IF initial_JUL GE lower_JUL AND final_JUL LE upper_JUL THEN $
                making_deltaH_plot, [initial_year, initial_month, initial_day], [final_year, final_month, final_day],  $
...
;##############################################################################
;##############################################################################
PRO geomagixs_magneticindex_plotdh, initial, final, STATION=station, $
                                                QUIET=quiet, $
                                                FORCE_ALL=force_all, $
                                                REAL_TIME=real_time, $
                                                N_DAYS=n_days
...
;##############################################################################
        ...
        IF capable_to_update LT 1 THEN BEGIN
                IF not keyword_set(quiet) THEN BEGIN
                        PRINT, '        Data File Error: GMS_YYYYMMDD.delta_H.'+extention+' files not found!'
        ...
        FOR i = 0ll, N_ELEMENTS(make_update_file)-1 DO BEGIN
                IF make_update_file[i] EQ 1 THEN BEGIN
                        ...
                        IF not keyword_set(N_DAYS) THEN BEGIN
                                making_dhplot, [tmp_year, tmp_month, tmp_day], N_DAYS=1, STATION=station, QUIET=quiet, REAL_TIME=real_time
                                making_dhplot, [tmp_year, tmp_month, tmp_day], N_DAYS=3, STATION=station, QUIET=quiet, REAL_TIME=real_time
                                making_dhplot, [tmp_year, tmp_month, tmp_day], N_DAYS=7, STATION=station, QUIET=quiet, REAL_TIME=real_time
                        ENDIF ELSE making_dhplot, [tmp_year, tmp_month, tmp_day], N_DAYS=n_days, STATION=station, QUIET=quiet, REAL_TIME=real_time
                ENDIF
        ENDFOR
