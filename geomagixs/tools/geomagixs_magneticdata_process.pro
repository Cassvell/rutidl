...
;##############################################################################
        ...
        IF not keyword_set(quiet) THEN BEGIN
                ...
                IF keyword_set(real_time) THEN print, FORMAT='("                      EARLY MODE: using data from previous month.")' $
                                          ELSE print, FORMAT='("                      FINAL MODE: using data from previous, requested, and future months.")'
        ENDIF

        CALDAT, JULDAY(initial_month, initial_day, initial_year), tmp_M, tmp_D, tmp_Y
        ...
        IF exist_kmex_file THEN BEGIN
                ...
                total_magnetic_data[minutes_per_day*(N_days-1):minutes_per_day*(N_days)-1].D = magnetic_data_tmp[*].D
                total_magnetic_data[minutes_per_day*(N_days-1):minutes_per_day*(N_days)-1].H = magnetic_data_tmp[*].H
                total_magnetic_data[minutes_per_day*(N_days-1):minutes_per_day*(N_days)-1].Z = magnetic_data_tmp[*].Z
                total_magnetic_data[minutes_per_day*(N_days-1):minutes_per_day*(N_days)-1].F = magnetic_data_tmp[*].F
        
                total_magnetic_data[minutes_per_day*(N_days-1):minutes_per_day*(N_days)-1].year   = magnetic_data_tmp[*].year
                total_magnetic_data[minutes_per_day*(N_days-1):minutes_per_day*(N_days)-1].month  = magnetic_data_tmp[*].month
                total_magnetic_data[minutes_per_day*(N_days-1):minutes_per_day*(N_days)-1].day    = magnetic_data_tmp[*].day
                total_magnetic_data[minutes_per_day*(N_days-1):minutes_per_day*(N_days)-1].hour   = magnetic_data_tmp[*].hour
                total_magnetic_data[minutes_per_day*(N_days-1):minutes_per_day*(N_days)-1].minute = magnetic_data_tmp[*].minute
        ENDIF ELSE BEGIN
                IF NOT keyword_set(quiet) THEN BEGIN
        ...
        qday_data     = geomagixs_quietday_get(initial, STATION=station, QUIET=quiet, REAL_TIME=real_time, LOCAL=local)
        qd_median_D   = MEDIAN(qday_data[*].D)
        qd_stddev_D   = STDDEV(qday_data[*].D)
        qd_median_H   = MEDIAN(qday_data[*].H)
        qd_stddev_H   = STDDEV(qday_data[*].H)
        qd_median_Z   = MEDIAN(qday_data[*].Z)
        qd_stddev_Z   = STDDEV(qday_data[*].Z)
        qd_median_F   = MEDIAN(qday_data[*].F)
        qd_stddev_F   = STDDEV(qday_data[*].F)
        
        
;##############################################################################
; this section aims to remove variation tendencies on real-time computings
; added by 28th march 2023 and finished by 19th apr 2023
;##############################################################################
        tmp_H_median      = 0.
        tmp_D_median      = 0.
        tmp_Z_median      = 0.
        ...
        IF keyword_set(real_time) THEN BEGIN
                days_for_tendency = 14
                H_for_tendency    = FLTARR(days_for_tendency)+999999.0
                dH_for_tendency   = FLTARR(days_for_tendency)+999999.0
                D_for_tendency    = FLTARR(days_for_tendency)+999999.0
                dD_for_tendency   = FLTARR(days_for_tendency)+999999.0
                Z_for_tendency    = FLTARR(days_for_tendency)+999999.0
                dZ_for_tendency   = FLTARR(days_for_tendency)+999999.0
                F_for_tendency    = FLTARR(days_for_tendency)+999999.0
                dF_for_tendency   = FLTARR(days_for_tendency)+999999.0
                ...
                FOR i=0, days_for_tendency-1 DO BEGIN
                        ...
                        IF tmp_good_indexes_count GT 0 THEN BEGIN
                                H_for_tendency[i] = tmp_good_indexes_count GT 500 ? median(tmp_data.H[tmp_good_indexes]) : mean(tmp_data.H[tmp_good_indexes])
                                dH_for_tendency[i]= tmp_good_indexes_count GT 500 ? STDDEV(tmp_data.H[tmp_good_indexes]) : ABS(MAX(ABS(tmp_data.H[tmp_good_indexes]))-H_for_tendency[i])
                                D_for_tendency[i] = tmp_good_indexes_count GT 500 ? median(tmp_data.D[tmp_good_indexes]) : mean(tmp_data.D[tmp_good_indexes])
                                dD_for_tendency[i]= tmp_good_indexes_count GT 500 ? STDDEV(tmp_data.D[tmp_good_indexes]) : ABS(MAX(ABS(tmp_data.D[tmp_good_indexes]))-D_for_tendency[i])
                                Z_for_tendency[i] = tmp_good_indexes_count GT 500 ? median(tmp_data.Z[tmp_good_indexes]) : mean(tmp_data.Z[tmp_good_indexes])
                                dZ_for_tendency[i]= tmp_good_indexes_count GT 500 ? STDDEV(tmp_data.Z[tmp_good_indexes]) : ABS(MAX(ABS(tmp_data.Z[tmp_good_indexes]))-Z_for_tendency[i])
                                F_for_tendency[i] = tmp_good_indexes_count GT 500 ? median(tmp_data.F[tmp_good_indexes]) : mean(tmp_data.F[tmp_good_indexes])
                                dF_for_tendency[i]= tmp_good_indexes_count GT 500 ? STDDEV(tmp_data.F[tmp_good_indexes]) : ABS(MAX(ABS(tmp_data.F[tmp_good_indexes]))-F_for_tendency[i])
                        ENDIF
...
;               FORMAT='(I02, "#   ----- ", F8.2, " ", F5.2, " ", F6.2, " ", F8.2, " ",I01, " ",I01, " ", F)'
                ...
                CASE 1 OF
                        good_values_count GE 7 : BEGIN
                                ...
                                tmp_result   = LINFIT( tmp_time[good_indxs], H_for_tendency[good_indxs], MEASURE_ERRORS=dH_for_tendency[good_indxs], YFIT=tmp_tendency )
                                tmp_H_median = INTERPOL( tmp_tendency, tmp_time[good_indxs], [1.*days_for_tendency] )
                                
                                tmp_result   = LINFIT( tmp_time[good_indxs], D_for_tendency[good_indxs], MEASURE_ERRORS=dD_for_tendency[good_indxs], YFIT=tmp_tendency )
                                tmp_D_median = INTERPOL( tmp_tendency, tmp_time[good_indxs], [1.*days_for_tendency] )
                                
                                tmp_result   = LINFIT( tmp_time[good_indxs], Z_for_tendency[good_indxs], MEASURE_ERRORS=dZ_for_tendency[good_indxs], YFIT=tmp_tendency )
                                tmp_Z_median = INTERPOL( tmp_tendency, tmp_time[good_indxs], [1.*days_for_tendency] )
                                
                                tmp_result   = LINFIT( tmp_time[good_indxs], F_for_tendency[good_indxs], MEASURE_ERRORS=dF_for_tendency[good_indxs], YFIT=tmp_tendency )
                                tmp_F_median = INTERPOL( tmp_tendency, tmp_time[good_indxs], [1.*days_for_tendency] )
                        ...
                        AND good_values_count GE 3 : BEGIN
                                good_indxs   = good_values[good_values_count-3:good_values_count-1]
                                ;tmp_result   = LINFIT( tmp_time[good_indxs], H_for_tendency[good_indxs]-mean(H_for_tendency[good_values]), MEASURE_ERRORS=dH_for_tendency[good_indxs], YFIT=tmp_tendency )
                                ;tmp_H_median = mean(H_for_tendency[good_indxs])-mean(H_for_tendency[good_indxs])
                                tmp_H_median = mean(H_for_tendency[good_indxs])
                                tmp_D_median = mean(D_for_tendency[good_indxs])
                                tmp_Z_median = mean(Z_for_tendency[good_indxs])
                                tmp_F_median = mean(F_for_tendency[good_indxs])
                        END
                
                        ELSE:   tmp_H_median   = 0.
                ENDCASE

                IF tmp_H_median NE 0. THEN BEGIN
                        ;print, tmp_H_median-qd_median_H
                        ;print, tmp_D_median-qd_median_D
                        ;print, tmp_Z_median-qd_median_Z
                        ;print, tmp_F_median-qd_median_F
                        ;qd_median_H   = Where(qday_data[*].H)
                        ;print, N_ELEMENTS(qday_data[*].H)
                        tmp_H_median = tmp_H_median-qd_median_H
                        tmp_D_median = tmp_D_median-qd_median_D
                        tmp_Z_median = tmp_Z_median-qd_median_Z
                        tmp_F_median = tmp_F_median-qd_median_F
                        ;qday_data[*].H = temporary(qday_data[*].H) + tmp_H_median-qd_median_H
                        ;qday_data[*].D += (tmp_D_median-qd_median_D)
                        ;qday_data[*].Z += (tmp_Z_median-qd_median_Z)
                        ;qday_data[*].F += (tmp_F_median-qd_median_F)
                ENDIF
        ENDIF
...
;##############################################################################
        ...
        valid_minutes    = WHERE(total_magnetic_data[*].H LT 999990.00, COUNT )
        
        IF COUNT GT 0 THEN BEGIN
                magnetic_data[valid_minutes].D = total_magnetic_data[valid_minutes].D
                D_median[valid_minutes]        = qday_data[valid_minutes].D + tmp_D_median
                D_sigma[valid_minutes]         = qday_data[valid_minutes].dD
                D_D[valid_minutes]             = (magnetic_data[valid_minutes].D - D_median[valid_minutes])
        
                magnetic_data[valid_minutes].H = total_magnetic_data[valid_minutes].H
                H_median[valid_minutes]        = qday_data[valid_minutes].H + tmp_H_median
                H_sigma[valid_minutes]         = qday_data[valid_minutes].dH
                D_H[valid_minutes]             = (magnetic_data[valid_minutes].H - H_median[valid_minutes])
        
                magnetic_data[valid_minutes].Z = total_magnetic_data[valid_minutes].Z
                Z_median[valid_minutes]        = qday_data[valid_minutes].Z + tmp_Z_median
                Z_sigma[valid_minutes]         = qday_data[valid_minutes].dZ
                D_Z[valid_minutes]             = (magnetic_data[valid_minutes].Z - Z_median[valid_minutes])
        
                magnetic_data[valid_minutes].F = total_magnetic_data[valid_minutes].F
                F_median[valid_minutes]        = qday_data[valid_minutes].F + tmp_F_median
...
;print, number_of_gaps        ;gaps    = where(D_values GE 9999.00)
        ...
        test_index = WHERE(D_D[*] LT 9990.0 , test_count)
        IF test_count GT 0 THEN BEGIN
                ...
                bad_index  = WHERE(D_D[*] GT 9990.0 OR D_N[*] GT 999990.0 OR $
                                   ...
                                   bad_count)
        ENDIF ELSE bad_index  = WHERE(D_D[*] GT 9990.0 OR D_sigma[*] GT 9990., bad_count) ;OR ABS(qday_data[*].D-qd_median_D) GE 1.5*qd_stddev_D, bad_count)
        ...
        FOR i=0, minutes_per_day-1 DO BEGIN
                ;print, i mod 60, i/60, D_median[i], qday_data[i].D, magnetic_data[i].D
        ...
        FOR i =0, data_per_day-1 DO BEGIN
                ...
                IF valid_count GT 0 THEN BEGIN
                        ...
                        delta_D[i] = (max_D[i] LT 9990.0 AND min_D[i] LT 9990.0) ? ABS(max_D[i]-min_D[i]) : 9999.00
                        sigma_D[i] = (delta_D[i] LT 9990.0) AND tmp_count GT 2 ? STDDEV( tmp[valid_data_tmp] ) : 9999.0
                ENDIF
...
;##############################################################################
        ...
        FOR i = 0ll, N_ELEMENTS(make_update_file)-1 DO BEGIN
                IF make_update_file[i] EQ 1 THEN BEGIN
                        ...
                        ;print, station
                        IF station NE 'planetary' THEN making_processeddatafiles, [tmp_year, tmp_month, tmp_day], QUIET=quiet, STATION=station, REAL_TIME=real_time, STATISTIC_QD=statistic_qd, LOCAL=local $
