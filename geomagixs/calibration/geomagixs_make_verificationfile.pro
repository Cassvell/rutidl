...
;###############################################################################
; FUNCION AUXILIAR
FUNCTION fixing_datafile, file_date, STATION=station, $
                                        QUIET=quiet
...
;        if station EQ 1 THEN BEGIN
        processed_data = geomagixs_get_processeddataday([tmp_year,tmp_month,tmp_day], STATION=station)
        data_number    = N_ELEMENTS(processed_data[*].dH)
...
;##############################################################################
        ...
        FOR i=0, minutes_in_a_day-1 DO $;BEGIN
                ...
                final_file[i] = string(tmp_year, tmp_month, tmp_day, i/60, i mod 60, $
                                       processed_data[i].dD, processed_data[i].dH, processed_data[i].dZ,processed_data[i].dF, $
                                       FORMAT='(I4," ",I02," ",I02, X, I02, ":", I02, 2x, F7.2, X, F9.2, X, F9.2, X, F9.2)')
...
PRO geomagixs_make_verificationfile, initial, final, STATION=station, $
                                              QUIET=quiet, $
                                              FORCE_ALL=force_all
...
;##############################################################################
        ...
        FOR i = 0ll, file_number-1 DO BEGIN
                ...
                tmp_string=fixing_datafile([tmp_year, tmp_month, tmp_day], QUIET=quiet, STATION=station)
        ...
        file_name = gms[system.gms].code+'_'+string(initial_year, initial_month, initial_day, final_year, final_month, final_day, FORMAT='(I4,I02,I02,"-",I4,I02,I02)')+'.verification.m'
