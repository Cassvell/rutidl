...
;##############################################################################
        ...
        calibration_dir = system.datasource_dir+gms[system.gms].name+'/';'~/DATA/calibration'
                ...
                file_name = gms[system.gms].code+string(initial_day, tmp_string, initial_year MOD 1000, FORMAT='(I02,A3,".",I02)')+'m'
...
;##############################################################################
        ...
        IF number_of_lines-4 GE 1 THEN BEGIN
                resulting_data = REPLICATE(DataStruct, number_of_lines-4)
                
                IF STRLEN(magnetic_data[4]) GT 57 THEN FORMAT_0=FORMAT_2 $
                                                  ELSE FORMAT_0=FORMAT_1
...
FUNCTION geomagixs_get_magneticdataday, initial, STATION=station, $
                                    QUIET=quiet, $
                                    REAL_TIME=real_time, $
                                    FORCE_ALL=force_all
