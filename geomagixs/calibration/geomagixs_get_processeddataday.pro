...
;##############################################################################
;##############################################################################
FUNCTION getting_processeddata, initial, STATION=station, $
                                        QUIET=quiet
        On_error, 2
...
;##############################################################################
        ...
        ;data_file_name              = gms[system.gms].code+'_'+string_date+'.clean.dat'
        ;exist_data_file             = FILE_TEST(system.processed_dir+gms[system.gms].name+'/'+data_file_name)


        calibration_dir = system.processed_dir+gms[system.gms].name+'/';'~/DATA/calibration'


                file_name = gms[system.gms].code+'_'+string_date+'.data.early'
...
;print, gms[system.gms].code+'_'+string_date+'.clean.dat'
...
;print, magnetic_data[5];STRLEN(magnetic_data[5])
;##############################################################################
; extracting data
;##############################################################################
        DataStruct     = { dD : 0., dH : 0., dZ : 0., dF : 0., dN : 0. }
        resulting_data = REPLICATE(DataStruct, number_of_lines)
                         
        READS, magnetic_data, resulting_data, FORMAT='(146X, 2X, F7, X, F9, X, F9, X, F9, X, F9)'
...
FUNCTION geomagixs_get_processeddataday, initial, STATION=station, $
                                    QUIET=quiet, $
                                    REAL_TIME=real_time, $
                                    FORCE_ALL=force_all
...
;##############################################################################
        ...
        tmp        = getting_processeddata([tmp_year,tmp_month, tmp_day], STATION=station, QUIET=quiet)
