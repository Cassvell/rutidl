...
PRO scatter_plot;, idate, fdate
        ...
        input_dir  = '/home/c-isaac/Escritorio/proyecto/master_thesis/datos/tgm/list1.1/'
        path       = input_dir
...
;RETURN
        ...
        idx = WHERE(dst GE -50 AND dst LT 60)      
        j =   WHERE(dst LT -50 AND dst GE -210)

        idx2 = WHERE(dst GE -75 AND dst LT 60)      
        j2 =   WHERE(dst LT -75 AND dst GE -210)
 
        idx3 = WHERE(dst GE -100 AND dst LT 60)      
        j3 =   WHERE(dst LT -100 AND dst GE -210)
...
;###############################################################################      
   ...
   POSITION=[0.17,0.1,0.9,0.9], TITLE='Case 1', /NODATA                 
...
;###############################################################################
    med = texToidl('R^2 = ')
        XYOUTS, 0.2, 0.80 , /NORMAL, $
             ;   med, COLOR=negro, $
                string(med, 0.72, FORMAT='(A, F4.2)') , COLOR=negro, $
                CHARSIZE = 1.2, $
                CHARTHICK=chr_thick1   
                
    desv = texToidl('(100 \leq Dst \leq -75)')                
...
;###############################################################################
    ...
    desv = texToidl('(-75 \leq Dst \leq -250)')                
...
;-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-
        IF keyword_set(jpeg) THEN BEGIN
                ...
                write_jpeg, path+'dispersion_plotV_case1_1.jpg', True_Image, true=1
        ENDIF ELSE BEGIN
                IF NOT (keyword_set(quiet) OR keyword_set(png)) THEN print, '        Setting PNG as default file type.'
                WRITE_PNG, path+'dispersion_plotV_case1_1.png', Image, reds,greens,blues
        ENDELSE

        IF NOT keyword_set(quiet) THEN BEGIN
                print, '        Saving: '+path+'dispersion_plotV_case1_1.png'
                print, ''
        ENDIF
        RETURN 	
